...
import warnings
from visualize import plot_3d_space, plot_2d_space, plot_2d_TF, plot_2d_space
...
class SelfPacedEnsemble():
    ...
    def fit(self, X, y, label_maj=0, label_min=1):
        ...
        X_train, y_train = self._random_under_sampling(
            X_maj, y_maj, X_min, y_min)  # 多数类:少数类=1:1
        ...
        self.estimators_.append(
            self._fit_base_estimator(
                X_train, y_train))
        self._y_pred_maj = self.predict_proba(X_maj)[:, 1]  # 分类器预测多数类
        ...
        for i_estimator in range(1, self._n_estimators):
            ...
            y_pred_maj_last_clf = self.estimators_[-1].predict_proba(X_maj)[
                                  :, 1]
