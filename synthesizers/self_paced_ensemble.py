...
class SelfPacedEnsemble():
    ...
    def _self_paced_under_sampling(self,
        ...
        # print(hardness.max() == hardness.min())
        ...
        # Else allocate majority samples into k hardness bins
            ...
            for i_bins in range(self._k_bins):
                ...
                # print(i_bins,ave_contributions)
            ...
            X_train_maj = torch.cat(sampled_bins, dim=0)
            ...
            if sp.issparse(X_min):
                ...
                X_train = torch.cat((X_train_maj, X_min), dim=0)
    ...
    def fit(self, X, y, label_maj=0, label_min=1):
        ...
        # print('==================')
        # print("X_maj:",np.shape(X_maj))
        # print("X_min:",np.shape(X_min))
        # Random under-sampling in the 1st round (cold start)
        X_train, y_train = self._random_under_sampling(
            X_maj, y_maj, X_min, y_min)

        self.X_train, self.y_train = X_train, y_train
        # print("X_train_spe:",np.shape(X_train))
        # if(np.shape(X_train == 2)):
        #     plot_2d_space2(X_train, y_train, xlimits=[-5, 10], ylimits=[-5, 5], label='generated1--spe')
        self.estimators_.append(
            self._fit_base_estimator(
                X_train.detach().cpu().numpy(), y_train))

        self._y_pred_maj = self.predict_proba(X_maj.detach().cpu().numpy())[:, 1]
        ...
        for i_estimator in range(1, self._n_estimators):
            ...
            self.y_train = np.append(self.y_train, y_train, axis=0)
            # if (np.shape(X_train == 2)):
            #     plot_2d_space2(X_train, y_train, xlimits=[-5, 15], ylimits=[-5, 15], label='generated1--{i}'.format(i=i_estimator+1))
            self.estimators_.append(
                self._fit_base_estimator(
                    X_train.detach().cpu().numpy(), y_train))
            # update predicted probability
            n_clf = len(self.estimators_)
            y_pred_maj_last_clf = self.estimators_[-1].predict_proba(X_maj.detach().cpu().numpy())[
                                  :, 1]
            self._y_pred_maj = (self._y_pred_maj *
                                (n_clf - 1) + y_pred_maj_last_clf) / n_clf
            # print('self',self._y_pred_maj)
        return self
