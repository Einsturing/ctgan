...
def benchmark(y_true, y_pred):
    ...
    print("tp + fn: ", len([y_true for y_true in y_true if y_true == 1]), ":", len(y_true))
    print("tp + fp: ", len([y_pred for y_pred in y_pred if y_pred == 1]), ":", len(y_pred))
    y_merg = y_pred * y_true
    print("tp: ", len([y_merg for y_merg in y_merg if y_merg == 1]), ":", len(y_merg))
...
def benchmark_inner(y_true, y_pred):
    auc = roc_auc_score(y_true, y_pred)
    return auc
